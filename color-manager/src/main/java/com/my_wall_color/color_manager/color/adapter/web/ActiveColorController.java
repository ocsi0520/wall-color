...
public class ActiveColorController {
    ...
    @PostMapping("/{id}")
    public ResponseEntity<Void> assignColorToUser(@PathVariable("id") int colorId, Principal principal) {
        try {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> revokeColorFromUser(@PathVariable("id") int colorId, Principal principal) {
        try {
