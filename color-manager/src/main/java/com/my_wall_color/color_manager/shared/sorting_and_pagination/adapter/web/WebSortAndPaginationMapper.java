...
public class WebSortAndPaginationMapper<F extends Enum<F>> {
  ...
  public SortAndPagination<F> map(Pageable pageable) {
    ...
    List<SortOrder<F>> nonEmptyOrders =
        filteredMappedSortOrder.isEmpty() ? List.of(defaultSortOrder) : filteredMappedSortOrder;
  ...
  private Optional<SortOrder<F>> mapToDomainSortOrder(Sort.Order order) {
    ...
    SortOrder.Direction direction =
        order.isAscending() ? SortOrder.Direction.ASCENDING : SortOrder.Direction.DESCENDING;
