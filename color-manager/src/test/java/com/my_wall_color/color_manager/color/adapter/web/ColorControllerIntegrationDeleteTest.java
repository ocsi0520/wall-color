...
class ColorControllerIntegrationDeleteTest extends IntegrationTest {
  ...
  void shouldReturnNotFoundForNonExistentColor() {
    ...
    var response =
        restTemplate.exchange("/api/color/" + colorFixture.nonExistent.getId(), HttpMethod.DELETE,
            entity, Void.class);
  ...
  void shouldNotBeAbleToDeleteWithoutAdminRole() {
    ...
    var entity = new HttpEntity<>(headerWithAuth);
    var response =
        restTemplate.exchange("/api/color/" + colorFixture.sulyom.getId(), HttpMethod.DELETE,
            entity, Void.class);
  ...
  void shouldBeAbleToDeleteColor() {
    ...
    var deleteResponse =
        restTemplate.exchange("/api/color/" + sulyom.getId(), HttpMethod.DELETE, entity,
            Void.class);
    assertThat(deleteResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

    var fetchResponse =
        restTemplate.exchange("/api/color/" + sulyom.getId(), HttpMethod.GET, entity, Color.class);
  ...
  private void testColorRevokedFromUser(User user, Color deletedColor) {
    ...
    var entity = new HttpEntity<>(headerWithAuth);
    ResponseEntity<List<Color>> assignedColorsResponse =
        restTemplate.exchange("/api/me/active-color", HttpMethod.GET, entity,
            new ParameterizedTypeReference<>() {
            });
    var colorStillAssigned = assignedColorsResponse.getBody().stream()
        .anyMatch(color -> deletedColor.getId().equals(color.getId()));
