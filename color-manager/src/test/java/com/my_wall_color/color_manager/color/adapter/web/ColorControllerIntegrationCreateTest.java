...
class ColorControllerIntegrationCreateTest extends IntegrationTest {
  ...
  void shouldNotAllowInsertingExistingColor() {
    ...
    ColorCreationRequest existingColorRequest =
        new ColorCreationRequest(existingColor.getRed(), existingColor.getGreen(),
            existingColor.getBlue(), existingColor.getName());
    ...
    ResponseEntity<Color> response =
        restTemplate.exchange("/api/color", HttpMethod.POST, entity, Color.class);
  ...
  void shouldNotAllowInvalidColorComponentRequest() {
    ...
    ResponseEntity<Color> invalidRedResponse =
        restTemplate.exchange("/api/color", HttpMethod.POST, invalidRedEntity, Color.class);
  ...
  void shouldNotAllowColorRequestWithEmptyName() {
    ...
    ResponseEntity<Color> emptyNameResponse =
        restTemplate.exchange("/api/color", HttpMethod.POST, emptyNameEntity, Color.class);
  ...
  void shouldCreateNewColor() {
    ...
    var fetchedNewColorResponse =
        restTemplate.exchange(response.getHeaders().getLocation(), HttpMethod.GET,
            new HttpEntity<>(authIncludedHeaders), Color.class);
    assertThat(fetchedNewColorResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
    testEqualityBetweenColorAndRequest(fetchedNewColorResponse.getBody(), validRequest,
        userFixture.jdoe);
  }

  private void testEqualityBetweenColorAndRequest(Color color, ColorCreationRequest request,
                                                  User expectedCreator) {
