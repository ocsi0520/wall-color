...
class ActiveColorControllerIntegrationTest extends IntegrationTest {
  ...
  private ResponseEntity<List<Color>> getActiveColorsFor(User user) {
    ...
    return restTemplate.exchange("/api/me/active-color", HttpMethod.GET, authIncludedEntity,
  ...
  void shouldReturnListForJdoe() {
    ...
    assertThat(actual.getBody()).isEqualTo(
        colorFixture.getInitialAssignedColorsFor(userFixture.jdoe));
  }

  @Test
  void shouldNotAssignNonExistingColor() {
    var authIncludedEntity =
        new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.jdoe));
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + colorFixture.nonExistent.getId(),
            HttpMethod.POST, authIncludedEntity, Void.class);
  ...
  void shouldNotAssignAlreadyAssignedColor() {
    var authIncludedEntity =
        new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.jdoe));
    var alreadyAssignedColor =
        colorFixture.getInitialAssignedColorsFor(userFixture.jdoe).getFirst();
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + alreadyAssignedColor.getId(),
            HttpMethod.POST, authIncludedEntity, Void.class);
  ...
  void shouldAssignValidColor() {
    var authIncludedEntity =
        new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.donna));
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + colorFixture.kekSzelloRozsa.getId(),
            HttpMethod.POST, authIncludedEntity, Void.class);
    assertThat(actual.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

    assertThat(getActiveColorsFor(userFixture.donna).getBody()).contains(
  ...
  void shouldLimitAssignmentsTo7() {
    var authIncludedEntity =
        new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.alex));
    ...
    for (Color color : firstSevenColors) {
      ResponseEntity<Void> assignResponse =
          restTemplate.exchange("/api/me/active-color/" + color.getId(), HttpMethod.POST,
              authIncludedEntity, Void.class);
      assertThat(assignResponse.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
    }
    var eigthColor = allFixtureColors.get(7);
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + eigthColor.getId(), HttpMethod.POST,
            authIncludedEntity, Void.class);
    assertThat(actual.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);

    for (Color color : firstSevenColors) {
      ResponseEntity<Void> revokeResponse =
          restTemplate.exchange("/api/me/active-color/" + color.getId(), HttpMethod.DELETE,
              authIncludedEntity, Void.class);
  ...
  void shouldNotRevokeNonExistingColor() {
    ...
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + colorFixture.nonExistent.getId(),
            HttpMethod.DELETE, authIncludedEntity, Void.class);
    assertThat(actual.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
  }
  ...
  void shouldRevokeColor() {
    ...
    ResponseEntity<Void> actual =
        restTemplate.exchange("/api/me/active-color/" + firstAssignedColor.getId(),
            HttpMethod.DELETE, authIncludedEntity, Void.class);
    assertThat(actual.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
