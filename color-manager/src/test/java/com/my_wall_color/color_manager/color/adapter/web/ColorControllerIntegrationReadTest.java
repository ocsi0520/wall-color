...
class ColorControllerIntegrationReadTest extends IntegrationTest {
  ...
  private @NotNull List<Color> getAllColorsInInsertionOrder() {
    return colorFixture.getAllFixtureColors().stream().sorted(Comparator.comparing(Color::getId))
        .toList();
  }

  private @NotNull List<Color> getAllColorsSortedByName() {
    return colorFixture.getAllFixtureColors().stream().sorted(Comparator.comparing(Color::getName))
        .toList();
  }
  ...
  public void shouldReturnNotFound() {
    ...
    ResponseEntity<Color> response =
        restTemplate.exchange("/api/color/" + colorFixture.nonExistent.getId(), HttpMethod.GET,
            entity, Color.class);
  ...
  private String generateSearchQuery(String pageIndex, String pageSize, String... sortOrders) {
    var sort =
        Arrays.stream(sortOrders).map(order -> "sort=" + order).collect(Collectors.joining("&"));
  ...
  public void shouldReturnSulyomColor() {
    var entity = new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.jdoe));
    ResponseEntity<Color> response =
        restTemplate.exchange("/api/color/" + colorFixture.sulyom.getId(), HttpMethod.GET, entity,
            Color.class);
  ...
  public void shouldReturnLast3ColorsByName() {
    var entity = new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.jdoe));
    ResponseEntity<PageDto<Color>> response =
        restTemplate.exchange("/api/color?page=0&size=3&sort=name,desc", HttpMethod.GET, entity,
            new ParameterizedTypeReference<>() {
            });
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    var actual = response.getBody();
    var expectedContent =
        List.of(colorFixture.szarkalab, colorFixture.sulyom, colorFixture.palastfu);
  ...
  public void shouldReturnDefaultPage() {
    var entity = new HttpEntity<>(authTestHelper.getAuthIncludedHeadersFor(userFixture.jdoe));
    ResponseEntity<PageDto<Color>> response =
        restTemplate.exchange("/api/color", HttpMethod.GET, entity,
            new ParameterizedTypeReference<>() {
            });
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    var actual = response.getBody();
  ...
  public void shouldHandleFirstExploitTryProperly() {
    ...
    ResponseEntity<PageDto<Color>> response = restTemplate.exchange(
        "/api/color" + suspiciousSearchQuery, HttpMethod.GET, entity,
        new ParameterizedTypeReference<>() {
        }
    );
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
