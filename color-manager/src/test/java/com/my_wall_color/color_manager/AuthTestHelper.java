...
public class AuthTestHelper {
  private static final String JWT_FROM_JWT_IO =
  ...
  public ResponseEntity<String> getAuthResponseFor(User user) {
    LoginRequest loginRequest =
        new LoginRequest(user.getUsername(), userFixture.getPasswordFor(user));
  ...
  private String retrieveTokenFrom(ResponseEntity<?> response) {
    ...
    return cookies.stream()
        ...
        .map(c -> c.substring(TOKEN_COOKIE_NAME.length() + 1,
            c.contains(";") ? c.indexOf(';') : c.length()))
        .findFirst()
        .orElse(null);
  }
}
